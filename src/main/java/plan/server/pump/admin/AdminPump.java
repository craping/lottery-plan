...
public class AdminPump extends DataPump<JSONObject, FullHttpRequest, Channel> {
	...
	public Errcode login (JSONObject params) {
		...
		LotteryAdmin user = userServer.getAdminUser(userName, Coder.encryptMD5(userPwd));
		...
		if (userMap == null || userMap.isEmpty()) {
			...
			userMap.put("user_role", user.getUserRole().toString());
		} else {
		...
		int result = userServer.updateAdminUser(user.getId(), new_token);
		if (result == 1) {
